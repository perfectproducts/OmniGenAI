...
import genai.service_setup_extension
from genai.service_setup_extension.service import router
...
class Test(omni.kit.test.AsyncTestCaseFailOnLogError):
    ...
    async def test_router_initialization(self):
        ...
        routes = [route for route in router.routes if route.path == "/text_to_image"]
        self.assertTrue(len(routes) > 0, "The text_to_image endpoint should be registered in the router")
